...
<ol class="breadcrumb">
    <li><a href="<?php echo base_url('contrato')?>"><span class="fa fa-file-text-o"></span> Contratos por proceso</a></li>
    <li class="active">Contratos por procesos</li>
</ol>
    <div class="panel panel-default">

        <div class="panel-body" style="background-color: #DC901E">
            <?php $attributes = array('id' => 'fcontrato', 'role' => 'form', 'class' => "form-inline");
            echo form_open_multipart('Reportes/contratos_por_proceso/', $attributes);
            ?>

            <select style="width: 30%" class="form-control input-sm" name="id_proceso" required="true">
                <option value="">Seleccione un proceso</option>
                <?php foreach ($datos as $row): ?>
                    <option value="<?php echo $row['id_proceso']; ?>"><?php echo $row['proceso']; ?></option>
                <?php endforeach; ?>
            </select>
    ...
    <?php if($firmados == true):?>
        ...
        <?php if (count($por_proceso) < 1): ?>
            <hr>
            <?php echo 'No hay datos a mostrar'; ?>
        <?php endif; ?>    
    
        <?php if (is_array($por_proceso) && count($por_proceso) > 0): ?>            
            <div class="page-header">
                <h2>Contrato(s) por el proceso: <small><?php echo $por_proceso[0]['proceso']; ?></small></h2>
            </div>
            ...
            <table id="example" class="table table-hover">
                ...
                <tbody>
                    <?php foreach ($por_proceso as $row): ?>
                        <tr>
