...
class Contrato extends CI_Controller {
    ...
    public function modificar_contrato($id_contrato){ // muestra vista para modificar un contrato   
        $correcto = $this->utilidades->chequear_loggin();
        if($correcto <> true){  
            $permiso = '';
            if(in_array("48", $this->session->userdata('id_rol'))){
	...
	public function suplementar_contrato($id_contrato){ // muestra vista para modificar un contrato   
        $correcto = $this->utilidades->chequear_loggin();
        if($correcto <> true){  
            $permiso = '';
            if(in_array("48", $this->session->userdata('id_rol'))){
    ...
    public function muestra_empresas(){ // muestra listado de empresas en dependencia del tipo de empresa seleccionado
    ...
    public function cancelar($id) { // cambia a 0 el estado de un contrato
...
//    }    
    ...
    public function modificar($contrato_id) { // modifica un contrato existente      
	...
	public function suplementar($contrato_id) { // suplemento a  un contrato existente      
    ...
    public function add() { // agrega un nuevo contrato       
        $subirPDF = $this->contrato->do_upload();
        $dfichero = $this->upload->data();
        $nfichero = explode(".", $dfichero['file_name']);
